...
For $\Leftarrow$ use the combinatorial lemma \ref{countable_constraint_sequence} to construct a sequence $\mathcal{M}_1 \subset \mathcal{M}_2 \subset ...$ such that
\begin{itemize}
    \item each $\mathcal{M} \in \mathcal{K}$ embeds into some $\mathcal{M}_k$.
    \item for embeddings $f: \mathcal{A} \to \mathcal{M}_n, \ g: \mathcal{A} \to \mathcal{B}$ with $\mathcal{A}, \mathcal{B} \in \mathcal{K}$ have embedding $h: \mathcal{B} \to \mathcal{M}_n$ such that $\mathcal{M} \subset \mathcal{M}_k$ and $h \circ g = f$.
\end{itemize}
