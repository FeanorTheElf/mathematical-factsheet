...
\end{equation*}
Then $f: \mathrm{On} \to \mathrm{On}, \ \alpha \mapsto \mathrm{ord}(\alpha \times \alpha)$ fulfils the conditions from the lemma.$\hfill\square$
...
\end{equation*}
Then the function $\mathrm{On} \to \mathrm{On}, \ \alpha \mapsto \mathrm{ord}(\alpha^\gamma)$ fulfils the conditions of the modified lemma, and the claim follows as $\mathrm{card}(X) \geq 2^\gamma$.$\hfill\square$

\subsection{Ordinal arithmetic}
For $\alpha, \beta \in \mathrm{On}$ define $\alpha + \beta := \mathrm{ord}((\{0\} \times \alpha) \cup (\{1\} \times \beta))$ (with lexicographic ordering).
...
\end{itemize}
Then define $\cdot$ by $\alpha \cdot \beta := \mathrm{ord}(\alpha \times \beta)$ (with lexiographic ordering).
...
\subsection{Main theorem of finitly generated modules over PIDs}
...
If $R$ is a noetherian ring, then so is $R[s_1, ..., s_n]$ for $s_1, ..., s_n \in S$ with a finitly generated ring extension $S \supseteq R$.
...
\begin{description}
    \item[Bessel] For an orthonormal system $S$ have $\sum_{s \in S} |\langle x, s \rangle|^2 \leq \| x \|^2$ for all $x$
    \item[Parseval] $S$ is an orthonormal basis iff there is equality above, i.e. $\| x \|^2 = \sum_{s \in S} |\langle x, s \rangle|^2$
\end{description}
...
\begin{description}
    \item[Compact operators $\mathcal{K}(H)$] In $T = \sum_n s_n \langle \cdot, e_n \rangle f_n \in \mathcal{K}(H)$ have $(s_n)_n \in c_0$
    \item[Hilbert-Schmidt operators $\mathrm{HS}(H)$] Compact operators where $(s_n)_n \in \ell^2$
    \item[Nuclear operators $\mathcal{N}(H)$] Compact operators where $(s_n)_n \in \ell^1$
\end{description}
Have the corresponding norms $\| \cdot \|_{\mathrm{op}}, \| \cdot \|_{\mathrm{HS}}, \| \cdot \|_{\mathrm{nuk}}$ as the $\ell^\infty, \ell^2, \ell^1$-norms of the $(s_n)_n$.
Then
\begin{itemize}
    \item $\| \cdot \|_{\mathrm{op}} \geq \| \cdot \|_{\mathrm{HS}} \geq \| \cdot \|_{\mathrm{nuk}}$, so the identity embedding is continuous
    \item The nuclear operators can be defined as operators of the form $\sum y_i x'_i(\cdot)$ where $\sum \| y_i \| \| x'_i \|$ converges
    \item $(\mathcal{N}(H), \|\cdot\|_{\mathrm{nuk}})$ is a Banach space
    \item Nuclear operators have the ``ideal property'': $T \circ S \circ R \in \mathcal{N}(H)$ if $S \in \mathcal{N}(H)$
\end{itemize}
For a nuclear operator $T = \sum s_n \langle \cdot, e_n \rangle f_n$ and an orthonormal basis $E$ the series
\begin{equation*}
    \mathrm{tr}(T) := \sum_{e \in E} \langle Te, e \rangle = \sum s_n \langle f_n, e_n \rangle
\end{equation*}
is independent of the choice of $E$ and defines the trace of $T$. We then further get
\begin{itemize}
    \item For $T, S \in \mathrm{HS}(H)$ have $T \circ S \in \mathcal{N}(H)$ and $\| T \circ S \|_{\mathrm{nuk}} \leq \| T \|_{\mathrm{HS}} \| S \|_{\mathrm{HS}}$ (compare the Hölder inequality \ref{hoelder_inequality})
    \item $(\mathrm{HS}(H), \langle \cdot, \cdot \rangle_{\mathrm{HS}})$ defines a Hilbert space via $\langle x, y \rangle_{\mathrm{HS}} := \mathrm{tr}(T^* \circ S)$ (well-defined by the above point)
\end{itemize}
...
\begin{description}
    \item[8.2] Have $\sum e_i f_i = n := [L : K]$ where $\mathfrak{p}\mathcal{O} = \mathfrak{B}_1^{e_1}...\mathfrak{B}_r^{e_r}$ is the factorization of $\mathfrak{p}$ into prime ideals in $\mathcal{O}$ and $f_i = [\mathcal{O}/\mathfrak{B}_i : \mathcal{R}/\mathfrak{p}]$. 
    The proof uses the CRT and the properties of $\mathcal{O}/\mathfrak{B}_i$ as $\mathcal{R}/\mathfrak{p}$-vector space.
    \item[8.3] Let $L = K(\alpha)$ for an integral, primitive element $\alpha \in \mathcal{O}$. If $\mathfrak{p}$ is a prime ideal that does not divide the leader $\mathcal{F}$ of $\mathcal{R}[\alpha]$ (the largest ideal contained in $\mathcal{R}[\alpha]$), then 
    \begin{equation*}
        \mathfrak{p} = \mathfrak{B}_1^{e_1}...\mathfrak{B}_r^{e_r} \quad \text{for} \quad \mathfrak{B}_i = \mathfrak{p}\mathcal{O} + p_i(\alpha)\mathcal{O}
    \end{equation*}
    where the minimal polynomial $p$ of $\alpha$ splits into irreducible factors mod $\mathfrak{p}\mathcal{O}$ 
    \begin{equation}
...
\end{description}
If $L|K$ is galoisch, we can consider the effect of the Galois group $G$ on the prime ideals $\mathfrak{B} \leq \mathcal{O}$ over some prime ideal $\mathfrak{p} \leq \mathcal{R}$.
...
\end{equation*}
Let then be $e$ resp. $f$ be the ramification index (de: Verzweigungsindex) (maximal power such that $\mathfrak{B}^e | \mathfrak{p}$) resp. residual index (de: Trägheitsindex) (the index of $\mathcal{O}/\mathfrak{B} | \mathcal{R}/\mathfrak{p}$) of $\mathfrak{B}$ over $\mathfrak{p}$. 
Note that in general, these might be different for different $\mathfrak{B} | \mathfrak{p}$ but if $L|K$ is galoisch, they are the same for each prime ideal factor of $\mathfrak{p}$.
If $\mathcal{O}/\mathfrak{B} | \mathcal{R}/\mathfrak{p}$ is separable, have the following representation:
\begin{align*}
...
\end{align*}
where the ramification indices are written over the corresponding ideal decompositions and the residual indices are written below, respectively.
